...
class LinearInterpolationModule(nn.Module):
    ...
    def forward(self, x_new_):
        if not isinstance(x_new_, torch.Tensor):
            x_new_ = torch.tensor(x_new_, dtype=torch.float32)
    ...
    def get_similarity(self):
        ...
        for i in range(size-1):
            for j in range(i+1, size):
                ...
                i_index = indices[i]
                j_index = indices[j]
                this_loss = ANN.pearson_correlation(self.linterp([i_index]).reshape(-1), self.linterp([j_index]).reshape(-1))
                loss = loss+this_loss
        loss = loss/pairs
        return loss
...
class Algorithm_bsdrcnn_r2(Algorithm):
    ...
    def _fit(self):
        ...
        for epoch in range(self.total_epoch):
            ...
            similarity_loss = self.ann.get_similarity()
            loss = mse_loss + 10*similarity_loss
            loss.backward()
            optimizer.step()
            if self.verbose:
                self.report(epoch,mse_loss.item(),similarity_loss.item())
        return self
    ...
    def write_columns(self):
        if not self.verbose:
            return
        columns = ["epoch","mse","sim","r2","rmse","rpd","rpiq","train_r2","train_rmse","train_rpd","train_rpiq"] + [f"band_{index+1}" for index in range(self.target_size)]
        print("".join([str(i).ljust(20) for i in columns]))

    def report(self, epoch,mse,sim):
        if not self.verbose:
            return
        if epoch%10 != 0:
            return
        ...
        cells = [epoch, round(mse,3), round(sim,3), r2, rmse_o, rpd_o, rpiq_o, train_r2, train_rmse_o, train_rpd_o, train_rpiq_o] + bands
