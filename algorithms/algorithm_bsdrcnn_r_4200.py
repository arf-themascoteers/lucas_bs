...
class Algorithm_bsdrcnn_r_4200(Algorithm):
    def __init__(self, dataset, train_x, train_y, test_x, test_y, target_size, fold, scaler_y, mode, train_size, reporter, verbose):
        ...
        self.train_dataloader = data.DataLoader(self.train_dataset, batch_size=100, shuffle=True)
        self.test_dataloader = data.DataLoader(self.test_dataset, batch_size=100, shuffle=True)

        self.linterp_trains = []

        for batch_x, batch_y in self.train_dataloader:
            self.linterp_trains.append(LinearInterpolationModule(batch_x, self.device))

        self.linterp_tests = []

        for batch_x, batch_y in self.test_dataloader:
            self.linterp_tests.append(LinearInterpolationModule(batch_x, self.device))
    ...
    def _fit(self):
        ...
        for epoch in range(self.total_epoch):
            ...
            for index, (batch_x, batch_y) in enumerate(self.train_dataloader):
                optimizer.zero_grad()
                linterp_train = self.linterp_trains[index]
                y_hat = self.ann(linterp_train)
                mse_loss = self.criterion(y_hat, batch_y)
